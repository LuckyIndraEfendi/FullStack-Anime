...
const Details = () => {
  const [defaultUrlVideo, setDefaultUrlVideo] = useState();
  const [details, setDetails] = useState("");
  const [animeUrl, setAnimeUrl] = useState("");
  const { id } = useParams();
  React.useEffect(() => {
    const defaultUrl = async () => {
      const url = await axios.get(`${process.env.REACT_APP_BASEURL}/anime/gogoanime/servers/${id}-episode-1`);
      setDefaultUrlVideo(url.data[1].url);
    };
    defaultUrl();
    const details = async () => {
      const url = await axios.get(`https://gogoanime.consumet.stream/anime-details/${id}`);
      setDetails(url.data);
    };
    details();
  }, []);
                  ...
                  <div className="desc w-[95%] m-auto">
                    ...
                    <div className="px-4 py-3">
                      ...
                      <div className="flex gap-1 mt-2">
                        ...
                        <a href={`https://pinterest.com/pin/create/button/?url=${details.animeTitle}${window.location.href}`} target="_blank" className="bg-red-600 px-3 rounded-sm py-1 text-[11px] text-white">
                          Pinterest
                        </a>
                      </div>
